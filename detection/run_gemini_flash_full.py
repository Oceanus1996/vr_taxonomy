"""
Gemini 2.5 Flash-Lite Experiment for Table 5: Taxonomy-Guided Fault Localization.
...
Usage:
  python run_gemini_flash_full.py run         # Run experiment
  python run_gemini_flash_full.py analyze     # Print results
"""
...
def run():
    ...
    output_file = 'rq4_gemini25_flash_lite_top3_results.jsonl'
    done_keys = load_done_keys(output_file)
    print(f"Gemini 2.5 Flash-Lite: {len(existing)} bugs, {len(done_keys)} already done")
    ...
    for ex in existing:
        ...
        try:
            # --- A (L_bare) ---
            resp = call_llm_gemini(build_prompt_bare_top3(files_content))
            ...
            resp = call_llm_gemini(build_prompt_random_top3(files_content, flat_patterns))
            preds = parse_response_top3(resp)
            result.update({
            ...
            if pats_full:
                resp = call_llm_gemini(build_prompt_full_top3(files_content, pats_full, f"{symptom} x {layer}"))
                preds = parse_response_top3(resp)
            else:
                preds = []
            result.update({
...
def analyze():
    ...
    results = load_jsonl('rq4_gemini25_flash_lite_top3_results.jsonl')

    print(f"\n{'='*70}")
    print(f"Gemini 2.5 Flash-Lite Results (Table 5)")
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: python run_gemini_flash_full.py [run|analyze]")
        sys.exit(1)
